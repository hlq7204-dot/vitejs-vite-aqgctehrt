...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubFolders = onSnapshot(foldersRef, (snap) => {
      ...
      if (f.length === 0 && !snap.metadata.hasPendingWrites) {
         initialFolders.forEach(folder => setDoc(doc(foldersRef, folder.id), folder));
      } else { 
        setFolders(f); 
      }
    }, console.error);
    ...
    const unsubDecks = onSnapshot(decksRef, (snap) => {
      ...
      if (d.length === 0 && !snap.metadata.hasPendingWrites) {
         initialDecks.forEach(deck => setDoc(doc(decksRef, deck.id), deck));
      } else { 
        setDecks(d); 
      }
    }, console.error);
  ...
  const handleAnswer = (quality) => {
    ...
    const newDecks = decks.map(deck => {
      if (deck.id === activeDeckId) {
        const updatedDeck = { ...deck, cards: deck.cards.map(c => c.id === updatedCard.id ? updatedCard : c) };
      ...
      <div className="max-w-5xl mx-auto p-4 sm:p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div className="bg-indigo-500/10 text-indigo-300 px-4 py-2 rounded-full font-medium flex items-center gap-2 border border-indigo-500/20 shadow-[0_0_15px_rgba(99,102,241,0.1)]">
              <Trophy className="w-5 h-5" /> {totalPendingGlobal}
            </div>
            {isImporting ? (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-sm p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSavePomoSettings} className="space-y-5">
              <div>
                ...
                <input type="number" min="1" max="120" required value={pomoWorkDuration} onChange={(e) => setPomoWorkDuration(e.target.value === '' ? '' : parseInt(e.target.value))} className="w-full bg-slate-950 border border-slate-800 text-slate-100 rounded-xl p-3" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Tempo de Pausa (minutos)</label>
                <input type="number" min="1" max="60" required value={pomoBreakDuration} onChange={(e) => setPomoBreakDuration(e.target.value === '' ? '' : parseInt(e.target.value))} className="w-full bg-slate-950 border border-slate-800 text-slate-100 rounded-xl p-3" />
              </div>
              <div className="flex gap-3 mt-8 pt-2">
                <button type="button" onClick={() => setIsPomoSettingsOpen(false)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium py-3 rounded-xl">Cancelar</button>
                <button type="submit" className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-medium py-3 rounded-xl">Guardar</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4" onClick={() => setItemToDelete(null)}>
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-sm p-6 shadow-2xl text-center" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setItemToDelete(null)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium py-3 rounded-xl">Cancelar</button>
              <button onClick={confirmDelete} className="flex-1 bg-rose-600 hover:bg-rose-500 text-white font-medium py-3 rounded-xl">Eliminar</button>
            </div>
          </div>
        </div>
      )}
              ...
              <div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
                  ...
                  {modalType !== 'folder' && DECK_THEMES.map(theme => (
                    <button key={theme.id} type="button" onClick={() => setNewItemColor(theme.color)} className={`shrink-0 w-12 h-12 rounded-xl flex items-center justify-center transition-all ${theme.color} ${newItemColor === theme.color ? 'ring-2 ring-white ring-offset-2 ring-offset-slate-900 scale-110 z-10' : 'opacity-70 hover:opacity-100'}`} title={theme.label}>
